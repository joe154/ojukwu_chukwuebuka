  ...
  return (
    ...
    >
      <nav className="container-max px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
        ...
        >
          {navItems.map((item, index) => (
            <Link
              key={item.href}
              href={item.href}
              className="relative px-4 py-2 text-slate-300 hover:text-slate-100 font-medium transition-colors duration-200 group"
            >
              {item.label}
        ...
        >
          ...
          <button
            ...
            className="lg:hidden ml-2 p-2 hover:bg-slate-700/50 rounded-lg transition-colors duration-200 text-slate-300 hover:text-slate-100"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            className="lg:hidden border-t border-slate-700/30 bg-slate-800/50 backdrop-blur-xl"
          ...
          >
            <div className="container-max px-4 sm:px-6 py-4 flex flex-col gap-3">
              {navItems.map((item, index) => (
                ...
                >
                  <Link
                    href={item.href}
                    className="block px-4 py-3 text-slate-300 hover:text-slate-100 hover:bg-slate-700/50 rounded-lg transition-all duration-200 font-medium"
            ...
            <Link href="/about" className="text-gray-300 hover:text-blue-400 transition-colors duration-200 font-medium block">
              About
            </Link>
            <Link href="/contact" className="text-gray-300 hover:text-blue-400 transition-colors duration-200 font-medium block">
              Contact
            </Link>
            {user && (
              <Link href="/admin" className="text-gray-300 hover:text-blue-400 transition-colors duration-200 font-medium block">
                Admin
              </Link>
            )}
          </div>
