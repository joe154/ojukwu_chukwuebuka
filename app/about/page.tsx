    ...
    <section className="max-w-4xl mx-auto py-8">
      ...
      <div className="mt-8">
        ...
        <div className="space-y-4">
          {timeline.map((t) => (
            <motion.div key={t.year} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="p-4 bg-slate-800 rounded">
      ...
      <motion.div className="mt-8" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
        ...
        <div className="flex flex-wrap gap-2">
          {!skills
            ...
            : skills.map((s: any) => (
                <motion.span key={s.id} className="px-2 py-1 bg-slate-800 rounded text-sm" whileHover={{ scale: 1.03 }}>
                  {s.name}
                </motion.span>
              ))}
        </div>
